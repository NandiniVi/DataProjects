# Reading the dataset
df <- read.csv("C:\\Users\\Hayden\\Documents\\Adjusted_Data.csv", header = TRUE)
attach(df) 
head(df)
str(df)
...
ggplot(train_df, aes(x = leverage, y = residuals)) +
  ...
  labs(title = "Residuals vs Leverage",
       ...
       y = "Residuals") +
  theme_minimal()
# Group data by predicted values
...
# At the bottom, add the Durbin-Watson test:

# Perform the Durbin-Watson Test on the final model
